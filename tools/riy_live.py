...
Unified TUI for the routed-experts observe+act pipeline. Talks to the
vLLM REST API (``/v1/routed-experts/*``) — the same surface exposed by
``vllm.entrypoints.serve.routed_experts.api_router`` — so it works with
any of the three orthogonal modes:

  * ``--enable-routed-experts-stats``      (observe: counts + weight sums)
  * ``--enable-routed-experts-mask``       (act: reversible runtime mask)
  * ``--riy-expert-profile``               (act: load-time prune, VRAM)

Usage:
    python riy_live.py
    python riy_live.py --host localhost --port 8000
    python riy_live.py --interval 1.0 --block 16
    python riy_live.py --demo   # synthetic data, no vLLM needed

Keybindings:
    q         quit
    r         reset stats (POST /v1/routed-experts/stats/reset)
    e         export current mask as riy_filter.<ts>.json
    d         disable stats collection
    g         enable stats collection
    ...
    p         set prune level (prompted for threshold, 0=clear)
    ?         toggle help
"""
...
def fetch_stats(host, port, timeout=3.0) -> Stats:
    """GET /v1/routed-experts/stats and map to the dashboard's Stats model.

    The unified API returns aggregated counts and weight sums. Per-(layer,
    expert) cells are rebuilt from ``layer_expert_activation_counts`` and
    ``layer_expert_weight_sums``:

      * frequency  = count / sum(counts in that layer)   (per-layer share)
      * avg_gate   = weight_sum / count                  (contribution per
                                                          activation; 0 when
                                                          the expert never
                                                          fired)
    """
    url = f"http://{host}:{port}/v1/routed-experts/stats"
    try:
    ...
    try:
        ...
        s = Stats()
        s.error = f"bad json: {e}"
        return s

    s = Stats()
    s.token_total = int(data.get("total_tokens_processed", 0))
    s.timestamp = time.time()
    s.is_collecting = bool(data.get("is_collecting", True))

    layer_counts = data.get("layer_expert_activation_counts", {}) or {}
    layer_weights = data.get("layer_expert_weight_sums", {}) or {}

    for layer_str, exp_map in layer_counts.items():
        try:
            layer = int(layer_str)
        except (TypeError, ValueError):
            continue
        w_map = layer_weights.get(layer_str, {}) or {}
        total_freq = 0
        try:
            total_freq = sum(int(c) for c in exp_map.values())
        except (TypeError, ValueError):
            total_freq = 0
        for e_str, cnt in exp_map.items():
            try:
                expert = int(e_str)
                cnt = int(cnt)
            except (TypeError, ValueError):
                continue
            try:
                w = float(w_map.get(e_str, 0.0))
            except (TypeError, ValueError):
                w = 0.0
            freq = (cnt / total_freq) if total_freq > 0 else 0.0
            gate = (w / cnt) if cnt > 0 else 0.0
            s.experts[(layer, expert)] = ExpertData(
                frequency=freq,
                avg_gate=gate,
                count=cnt,
            )
...
def post_reset(host, port):
    url = f"http://{host}:{port}/v1/routed-experts/stats/reset"
    try:
        _http_json(url, method='POST', body=b'')
        return True
    except Exception:
        return False
...
def post_enable(host, port, enable=True):
    ...
    url = f"http://{host}:{port}/v1/routed-experts/stats/{endpoint}"
    try:
        _http_json(url, method='POST', body=b'')
        return True
    except Exception:
...
class Dashboard:
    def __init__(self, host, port, interval, block_size, demo=False):
        ...
        self.current    = Stats()
        self.previous   = Stats()
        self.mask       = set()   # runtime mask (from 'p' / API)
    ...
    def _fetch_model_name(self):
        ...
        try:
            ...
            status, raw = _http_json(url, timeout=2.0)
            data = json.loads(raw)
    ...
    def _fetch_mask(self):
        """Fetch current runtime mask from the API on startup."""
        try:
            url = f"http://{self.host}:{self.port}/v1/routed-experts/mask"
            status, raw = _http_json(url, timeout=2.0)
            data = json.loads(raw)
    ...
    def _handle_key(self, key, stdscr):
        ...
        if key in (ord('q'), ord('Q')):
            ...
            if post_enable(self.host, self.port, True):
                self.set_status("Stats collection enabled")
            else:
                self.set_status("Enable failed")
        elif key == ord('d'):
            if post_enable(self.host, self.port, False):
                self.set_status("Stats collection disabled")
            else:
                self.set_status("Disable failed")
        elif key in (ord('+'), ord('=')):
    ...
    def _prompt_prune(self, stdscr):
        ...
        if pct == 0:
            # Clear runtime mask entirely.
            self._send_clear_mask()
            self.mask = set()
            self.show_mask = False
            self.set_status("Runtime mask cleared")
            return

        self._apply_prune(pct)

    def _apply_prune(self, pct):
        """Rank experts by score, prune bottom N% (runtime mask)."""
        with self.lock:
            stats = self.current
        ...
        if not scored:
            self.set_status("No experts to rank")
            return

        # Sort ascending — lowest score = best prune candidates
        scored.sort()

        # Take bottom N% of experts
        n_prune = int(len(scored) * pct / 100.0)
        pruned = [(l, e) for _, l, e in scored[:n_prune]]

        # Send to vLLM
        self._send_mask(pruned)
        self.mask = set(pruned)
        self.show_mask = True
        self.set_status(
            f"Pruned {len(pruned)} experts ({pct}% of total) — mask active"
        )

    def _send_mask(self, pruned):
        """POST mask to the routed-experts API."""
        try:
            body = json.dumps(
                {"pruned_experts": [list(p) for p in pruned]}
            ).encode()
            _http_json(
                f"http://{self.host}:{self.port}/v1/routed-experts/mask",
                method='POST',
                body=body,
            )
    ...
    def _send_clear_mask(self):
        """DELETE mask via the routed-experts API."""
        try:
            _http_json(
                f"http://{self.host}:{self.port}/v1/routed-experts/mask",
                method='DELETE',
    ...
    def _draw(self, stdscr, stats, flash):
        ...
        collect = "on" if stats.is_collecting else "off"
        hdr = (f" riy live | {model} | {connected}{err} | "
               f"L:{stats.max_layer+1} E:{stats.max_expert+1} | "
               f"{self.interval:.1f}s | {age} | stats:{collect} | ?=help ")
        try:
        ...
        try:
            ...
            if len(stats.experts) > 0:
                ...
                runtime_count = len(self.mask)
                if runtime_count > 0:
                    total_pct = runtime_count * 100 / max(len(stats.experts), 1)
                    prune_info = f"  ACTIVE: {total_pct:.1f}% live:{runtime_count}X"
                else:
                    prune_info = ""
    ...
    def _draw_expert(self, stdscr, row, col, layer, expert, v,
        ...
        if is_masked:
            ch   = "X"  # runtime-masked (via TUI 'p' or API)
    ...
    def _draw_help(self, stdscr, h, w):
        lines = [
            ...
            "",
            "  Endpoints (vLLM REST API):",
            "    GET  /v1/routed-experts/stats",
            "    POST /v1/routed-experts/stats/{reset,enable,disable}",
...
def main():
    ...
    parser.add_argument("--port",     type=int, default=8000,
                        help="vLLM REST API port (default: 8000)")
